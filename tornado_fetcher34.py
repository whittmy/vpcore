...
def unicode_obj(obj, encoding='utf-8'):
    if isinstance(obj, str):
#        print('----unicode_obj is str---')
        return obj.decode(encoding)
#    print('----unicode_obj not str----')
    return obj
...
class Fetcher(object):
    ...
    def __init__(self, phantomjs_proxy='http://localhost:25555', user_agent='', pool_size=100, async=False, has_body=True, has_head=True):
        #print('---Fetcher init-------')
    ...
    @staticmethod
    def parse_option(default_options, url, user_agent, has_body, has_head, **kwargs):
        ...
        fetch['js_url'] = kwargs.get('js_url', False)
        fetch['js_url_callback'] = kwargs.get('js_url_callback', False)
        fetch['load_images'] = kwargs.get('load_images', False)
        
        fetch['has_body'] = has_body
        fetch['has_head'] = has_head
        return fetch

    def phantomjs_fetch(self, url, **kwargs):
        start_time = time.time()
        fetch = self.parse_option(self.default_options, url, user_agent=self.user_agent, has_body=self.has_body, has_head=self.has_head, **kwargs)
        request_conf = {
        ...
        def handle_response(response):
            ...
            try:
                result = json.loads(text(response.body).decode())
                if response.error:
