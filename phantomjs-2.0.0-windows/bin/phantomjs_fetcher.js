...
if (system.args.length !== 2) {
  ...
  service = server.listen(port, {
    ...
    page.onLoadFinished = function(status) {
      ...
      if (!script_executed && fetch.js_script && fetch.js_run_at !== "document-start") {
        script_executed = true;
        console.debug('running document-end script.');
        script_result = page.evaluateJavaScript(fetch.js_script);
      }
      
      console.debug('---onLoadFinished---------');
      if(!fetch.js_url_callback){
    ...
    page.onResourceReceived = function(response) {
      ...
      if (first_response === null && response.status != 301 && response.status != 302) {
        ...
        console.debug('-----------------first-response- 2------');
        console.debug(JSON.stringify(first_response, null, 4));
      }
      if (page_loaded) {
        console.debug("waiting "+wait_before_end+"ms before finished.");
        end_time = Date.now() + wait_before_end;
    ...
    page.onResourceError=page.onResourceTimeout=function(response) {
      ...
      if (first_response === null) {
        first_response = response;
                console.debug('-----------------first-response- 3------');
        console.debug(JSON.stringify(first_response, null, 4));
      }
      if (page_loaded) {
        console.debug("waiting "+wait_before_end+"ms before finished.");
    ...
    // send request
    if(fetch.js_url){
        page.open(fetch.url,  function() {            
            //引入外部js库
            page.includeJs(fetch.js_url, function() {
                if(fetch.js_url_callback){
                    script_result = page.evaluateJavaScript(fetch.js_url_callback);
